...
Contrastes de hipótesis: Dualidad en pruebas de hipótesis,  constrastes más potentes y uniformemente más potentes. Cocientes de verosimilitud.
...
Entonces un intervalo de confianza con nivel $1-\alpha$ de este parámetro, está dado por $RC_\delta^c$.
...
Supongamos que tenemos datos $X_1, \dots, X_n$ con distribución Bernoulli con parámetro $p$. 
...
```{r, warning=FALSE}
x <- matrix(pbinom(0:10, size = 10, prob = 0.3, 
                   lower.tail = FALSE), nrow = 1)
colnames(x) <- 0:10
...
$X = (X_1,\dots, X_n)\sim N(\mu,\sigma^2)$, $\theta = (\mu,\sigma^2)$ desconocido. Queremos probar
...
Suponga que $Y$ representa una sola observación con función de densidad dada por: 
...
$$\mathcal{L}(\mu) = (2\pi)^{-\dfrac{n}{2}} e^{-\dfrac{\sum(x_{j}-\mu)^{2}}{2}} = (2\pi)^{-\dfrac{n}{2}} e^{-\dfrac{\sum{x_{j}^{2}} -2n\mu \overline{x} + n\mu^2 }{2}}$$
...
$$\dfrac{\mathcal{L}(\mu_0)}{\mathcal{L}(\mu_1)} = \dfrac{e^{n\mu_{0} \overline{x} -\dfrac{n\mu_{0}^{2}}{2}}}{e^{n\mu_{1} \overline{x} -\dfrac{n\mu_{1}^{2}}{2}}} = e^{n\left( \overline{x}(\mu_0 - \mu_1 )  -\frac{1}{2}( \mu_{0}^2 - \mu_{1}^2 )    \right) } = e^{ n\overline{x}(\mu_0 - \mu_1 ) } \cdot e^{ -\frac{n}{2}( \mu_{0}^2 - \mu_{1}^2 ) } < k$$
