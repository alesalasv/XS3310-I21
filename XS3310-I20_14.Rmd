...
Contrastes de hipótesis: Dualidad en pruebas de hipótesis,  contrastes más potentes y uniformemente más potentes. Cocientes de verosimilitud.

---

# Dualidad en pruebas

Suponga que se tiene una prueba de hipótesis y se encuentra la región crítica (o región de rechazo) \(RC_\delta\) de un contraste \(\delta\) sobre \(\theta\) con nivel \(\alpha\). 

Entonces un intervalo de confianza con nivel \(\alpha\) de este parámetro, esta dado por \(RC_\delta^c\).
...
**Ejemplo** Supongamos que tenemos datos \(X_1, \dots, X_n\) con distribución Bernoulli con parámetro \(p\). 

Tenemos la hipótesis 

\[ H_0: p < p_0 \text{ vs } H_1: p\geq p_0.\]

El criterio de rechazo al nivel $\alpha$ es 

\[Y = \sum_{i=1}^nX_i\geq c.\]

El valor \(c\) debe ser el menor valor que cumpla que 

\begin{equation*}
\operatorname{Pr}\left(Y \geq c \mid p=p_{0}\right) \leq \alpha_{0}
\end{equation*}

Suponga que \(n=10\) , \(p_0 = 0.3\), \(Y=6\) y \(\alpha = 0.1\). Los valores de \(P(Y>y)\) son 

```{r, warning=FALSE}
x <- matrix(pbinom(0:10, size = 10, prob = 0.3, lower.tail = FALSE), nrow = 1)
colnames(x) <- 0:10
knitr::kable(x, digits = 4)
```

La regla decisión es "Rechazamos \(H_0\) si $Y \geq c$". En este caso rechazamos si \(Y \geq 5 \). 


Entonces el intervalo de confianza sería la región donde \(Y < 5\) o donde \(p-valor > \alpha\).
...
\[ \left(\bar X_n-t_{n-1,\alpha}\dfrac{s}{\sqrt n},\infty\right).\]

Rechazamos $H_0$ si 

\[\mu_0\leq \bar X_n-t_{n-1,\alpha }\dfrac{s}{\sqrt n} \Leftrightarrow T = \dfrac{\bar X_n -\mu_0}{s/\sqrt n}\geq t_{n-1,\alpha}\]
...
$$\mathcal{L}(\mu) = (2\pi)^{-\frac{n}{2}} \exp \left({-\dfrac{\sum(x_{j}-\mu)^{2}}{2}}\right) = (2\pi)^{-\frac{n}{2}} \exp \left({-\dfrac{\sum{x_{j}^{2}} -2n\mu \overline{x} + n\mu^2 }{2}}\right)$$
...
$$\dfrac{\mathcal{L}(\mu_0)}{\mathcal{L}(\mu_1)} = \dfrac{\exp \left({n\mu_{0} \overline{x} -\frac{n\mu_{0}^{2}}{2}}\right)}{\exp \left({n\mu_{1} \overline{x} -\frac{n\mu_{1}^{2}}{2}}\right)} = \exp \left({n\left( \overline{x}(\mu_0 - \mu_1 )  -\frac{1}{2}( \mu_{0}^2 - \mu_{1}^2 )    \right) }\right) = \exp \left({ n\overline{x}(\mu_0 - \mu_1 ) }\right) \cdot \exp \left({ -\frac{n}{2}( \mu_{0}^2 - \mu_{1}^2 ) }\right) < k$$
...
\begin{align*}
& \exp \left({ n\overline{x}(\mu_0 - \mu_1 ) }\right) \cdot \exp \left({ -\frac{n}{2}( \mu_{0}^2 - \mu_{1}^2 ) }\right) < k 
\Longrightarrow  \exp \left({ n\overline{x}(\mu_0 - \mu_1 ) }\right) < k \exp \left({\frac{n}{2}( \mu_{0}^2 - \mu_{1}^2 ) }\right) \\ 
& \Longrightarrow n\overline{x}(\mu_0 - \mu_1 ) < \ln{ \left(  k e^{\frac{n}{2}( \mu_{0}^2 - \mu_{1}^2 ) } \right)  } 
 \Longrightarrow \overline{x} > \frac{ \ln{ \left(  k e^{\frac{n}{2}( \mu_{0}^2 - \mu_{1}^2 ) } \right)  } }{n(\mu_0 - \mu_1 )} = k^{\prime} 
\end{align*}
...
> **Cociente de verosimilitudes monótono:** Una muestra aleatoria con función de verosimilitud $\mathcal{L}(\theta)$ tiene un **cociente de verosimilitudes monótono** (CVM) en un estadístico $T = T(X_{1}, X_{2}, ... , X_{n})$ si, para $\theta_0 < \theta_1$, el cociente $\dfrac{\mathcal{L}(\theta_0)}{\mathcal{L}(\theta_1)}$ es una función monótona de $T$. 
...
$$\dfrac{\mathcal{L}(\mu_0)}{\mathcal{L}(\mu_1)} = e^{ n\overline{x}(\mu_0 - \mu_1 ) } \cdot e^{ -\frac{1}{2}( \mu_{0}^2 - \mu_{1}^2 ) }$$
...
> Si la distribución tiene un CVM en el estadístico $T$, entonces el contraste UMP para $H_{0}: \theta = \theta_0$ contra $H_{1}: \theta > \theta_0$ o $H_{1}: \theta < \theta_0$ existe y este se puede expresar en términos de $T$, en lugar del cociente $\dfrac{\mathcal{L}(\theta_0)}{\mathcal{L}(\theta_1)}$. 
