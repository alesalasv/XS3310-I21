...
* ECM y propiedades de los estimadores: insesgado, eficiente, consistente y suficiente.
...
Sea $X_{1}, X_{2}, ... , X_{n}$ una muestra aleatoria sobre una población con media $\mu$ y variancia $\sigma^2$, pruebe que $\overline{X}$ y $S^2$ son estimadores insesgados para estimar $\mu$ y $\sigma^2$ respectivamente.
...
$F_{X_{\left(n\right)}}\left(x\right) = P(X_{(n)} \leq x) =P(\max(X_1,X_2,⋯,X_n)\leq x) = P(X_1 \leq x,X_2 \leq x,⋯,X_n\leq x )$
...
**NOTA:** Del resultado de esta integral encontramos que el valor esperado de una Potencial con parámetros $\alpha$ y $\beta$ sería $\frac{\alpha \beta}{\alpha + 1}$. 
...
$f_{X}\left(x\right) = \begin{cases} \frac{2\beta^2}{x^3} \quad si \quad x \geq \beta \\ 0 \quad si \quad x < \beta \end{cases}$
	
$F_{X}\left(x\right) = \begin{cases} 1-\left(\frac{\beta}{x}\right)^2 \quad si \quad x \geq \beta \\ 0 \quad si \quad x < \beta \end{cases}$
...
$f_{X_{\left(1\right)}}\left(x\right) = n\left[1-F_{X}\left(x\right)\right]^{n-1}f_{X}\left(x\right) = n\left[1-\left(1-\left(\frac{\beta}{x}\right)^2\right)\right]^{n-1}\frac{2\beta^2}{x^3}$
	
$= \frac{n\beta^{2n-2}}{x^{2n-2}} \cdot \frac{2\beta^2}{x^3} = \begin{cases} \frac{2n\beta^{2n}}{x^{2n+1}} \quad si \quad x \geq \beta \\ 0 \quad si \quad x < \beta \end{cases}$
...
Por otra parte podemos ver cómo $\hat{\beta}^{*} = \frac{2n-1}{2n}X_{\left(1\right)}$ sí es un estimador insesgado para $\beta$.
