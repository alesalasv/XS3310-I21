...
Si $X_1,\ldots, X_n$ es una muestra aleatoria de tamaño $n$, y la distribución tiene media $\mu$ y varianza $\theta=\sigma^2<0$. Entonces se cumple que 
...
Suponga que $a_n(Y_n-\theta) \xrightarrow{d} N(0,\sigma^{2})$. 

Si $g$ es una función tal que $g^{\prime}(\theta)\ne 0$, entonces
$$\dfrac{a_n}{g'(\theta)}[g(X_n)-g(\theta)] \xrightarrow{d} N(0,\sigma^2 (g^{\prime}(\theta))^{2})$$
...
Entonces por el método Delta $\sqrt{n}[g(\bar X_n)-g(\mu)]\xrightarrow{d}N(0,\sigma^2 (g^{\prime}(\theta))^{2})$

Para la función $g$ seleccionada se tiene que  $g'(\mu) = -\dfrac{1}{\mu^2}$. Entonces por el método Delta

$$ \sqrt{n}\bigg[\dfrac 1{\bar X_n}-\dfrac 1\mu\bigg]\xrightarrow{d}N(0,\dfrac{\sigma ^2}{\mu ^4}) $$


La varianza asintótica es $\dfrac{\sigma}{n\mu ^4}$
---
...
La varianza asintótica es $\dfrac{\theta}{n}$. 
...
**Después de la transformación, la normal no depende del paramétro.**
---
...
> **Teorema 1.1.** Si $\bar{X}_n$ es la media muestral de una muestra aleatoria de tamaño $n$ de una distribución normal de media $\mu$ y variancia $\sigma^2$, entonces $\bar{X}_n$ tiene una distribución normal con media $\mu$ y variancia $\sigma^2/n$. El decir, $\bar{X}_n \thicksim N(\mu,\sigma^2/n)$
...
**Nota:** 
La parte $b$ se puede usar para probar que $Var(S^2) = \frac{2\sigma^4}{n-1}$ cuando asumimos que $X_1, X_2, ..., X_n$ es una muestra aleatoria con distribución normal con media $\mu$ y variancia $\sigma^2$.
