...
## Desigualdad de Chebyshev (version 1)

Si $X$ es una v.a. con $\mathbb{E}[X^{2}]\leq \infty$, entonces para cualquier $\varepsilon>0$, se tiene que 
...
## Desigualdad de Chebyshev (version 2)

Si $\hat{\theta}$ es una v.a. que cumple que  $\operatorname{Var}(X)$ existe. Entonces para cada número  $t>0$

$$\operatorname{P}(|\hat{\theta}-\theta| \geq \varepsilon) \leq  \frac{\mathbb{E}[(\hat{\theta}-\theta)]}{\varepsilon^2}=\frac{\operatorname{MSE}(\hat{\theta})}{\varepsilon^{2}}$$
...
$$\operatorname{P}(|\hat{\theta}-\theta| \geq \varepsilon) \leq  \frac{\mathbb{E}[(\hat{\theta}-\theta)^2]}=\frac{\operatorname{MSE}(\hat{\theta})}{\varepsilon^{2}}$$

---

# Consistencia con estimadores insesgados

> **Teorema** Si $\hat{\theta}$ es un estimador de $\theta$ entonces $\hat{\theta}$ es un estimador consistente si:
...
**Tarea:** Pruebe que $\lim_{n\to +\infty}MSE(X_{(n)}) =0 $
---
...
**Es decir, si $\theta \stackrel{p}{\longrightarrow} \theta$ entonces $g(\hat{\theta}) \stackrel{p}{\longrightarrow} g(\theta)$**
---
...
> **Definición 4.1.** __Suficiencia_. Si $X_{1}, X_{2}, ... , X_{n}$ es una muestra aleatoria sobre una población con parámetro desconocido $\theta$ y función de densidad/probabilidad $f_{X}(x|\theta)$. Se dice que un estadístico $U = g(X_{1}, X_{2}, ... , X_{n})$ es **suficiente** para estimar $\theta$ si la distribución condicional de $X_{1}, X_{2}, ... , X_{n}$ dado $U=u$ es independiente de $\theta$. 
> $$ P(X_{1}, X_{2}, ... , X_{n} \vert U =u) \text{no depende de $\theta$.}$$

---

## Teorema de Factorización de Fisher

Otra forma de entender el resultado anterior es usando este teorema:


**Teorema**. Si $X_1,\dots,X_n$ es una muestra aleatoria de $f(X|\theta)$, el
parámetro $\theta$ es desconocido. Un estadístico $U=U(X_1,\dots,X_n)$ es
suficiente si y solo si 

$$f_n(X_1,\dots,X_n|\theta) = u(X_1,\dots,X_n)v(U(X_1,\dots,X_n),\theta)\;\forall \theta \in \mathbb R.$$

Las funciones $u$ y $v$  son no negativas. 
---

# Suficiencia 

Suficiencia se refiere a que el estadistico $U$ contiene toda la información necesaria para estimar $\theta$. Por eso el paramétro está junto con el estadistico en la función $v(U(X_1,\dots,X_n),\theta)$. 
...
Recordemos que bajo independencia $f(x_{1}, ... , x_{n}) = \prod_{j=1}^{n}f_{X_{j}}(x_{j}|p)$. Esta función también lleva el nombre de **función de verosimilitud** y se denota como $\mathcal{L}(x_{1}, ... , x_{n}|p)$ o también solo como $\mathcal{L}(p)$. 

\begin{align*}
\mathcal{L}(x_{1}, ... , x_{n}|p) =  
\prod_{j=1}^{n} p^{x_{j}}\left(1-p\right)^{1-x_{j}} 
&= p^{\sum_{j=1}^{n}x_{j}}\left(1-p\right)^{n-\sum_{j=1}^{n}x_{j}} \\
&= p^{u}\left(1-p\right)^{n-u}
\end{align*}
...
\begin{align*}
\mathcal{L}(x_{1}, ... , x_{n}|p) = p^{u}\left(1-p\right)^{n-u}
\end{align*}


Tome $u(x) = 1$,  $v(u,p) = p^{u}\left(1-p\right)^{n-u}$. Por el teorema de factorización de Fisher, $U = \sum_{j=1}^{n} X_{j}$ es suficiente para estimar $p$.
...
>**Teorema**. Si $U = U(X_1,\dots, X_n)$ es un estadístico suficiente para
>$\theta$, entonces el MLE $\hat\theta$ de $\theta$ depende de $X_1,\dots, X_n$
>solamente a través de $U$. Además, si $\hat \theta$ es suficiente entonces $\hat\theta$ es minimal.
...
\begin{align*}
\mathcal{L}(x_{1}, ... , x_{n}|\theta) 
&= \prod_{j=1}^{n} \frac{1}{\theta} \quad 0\leq x_{1}, ... , x_{n}\leq  \theta \\
&=  \frac{1}{\theta^n} \quad 0\leq x_{1}, ... , x_{n}\leq  \theta \\
&=  \frac{1}{\theta^n} \quad 0\leq \max{x_{1}, ... , x_{n}}\leq  \theta \\
&=  \frac{1}{\theta^n} \quad 0\leq x_{(n)}\leq  \theta 
\end{align*}

---

Otra forma es que ya habiamos demostrado que $U = X_{(n)} \sim Potencial(n,\theta)$, por lo que conocemos su función de densidad:

$f_{U}(u) = \frac{nu^{n-1}}{\theta^n}$
	
Con esto podemos encontrar la función de densidad marginal:
	
$f(x_{1}, ... , x_{n} | U = u) = \dfrac{\frac{1}{\theta^{n}}}{\dfrac{nu^{n-1}}{\theta^n}} = \dfrac{1}{nu^{n-1}}$
...
- Suponga que$X_1,\dots,X_n \sim Poisson(\lambda)$, ¿Cuál sería un estimador suficiente minimal para $\lambda$?  
