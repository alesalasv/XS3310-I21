...
Si $\hat{\theta}$ es una v.a. que cumple que  $\operatorname{Var}(\hat{\theta})$ existe. Entonces para cada número  $\varepsilon>0$
...
> **Definición 4.1.** _Suficiencia_. Si $X_{1}, X_{2}, ... , X_{n}$ es una muestra aleatoria sobre una población con parámetro desconocido $\theta$ y función de densidad/probabilidad $f_{X}(x|\theta)$. Se dice que un estadístico $U = g(X_{1}, X_{2}, ... , X_{n})$ es **suficiente** para estimar $\theta$ si la distribución condicional de $X_{1}, X_{2}, ... , X_{n}$ dado $U=u$ es independiente de $\theta$. 
