...
author: ""
date: "22/05/2020 (updated: `r Sys.Date()`)"
output:
...
![Método ciéntífico](https://cdn.thinglink.me/api/image/685791022688501762/1240/10/scaletowidth)	
...
$\text{Potencia}(1) = P(\overline{X} < 0 | \mu = 1) = P\left( \frac{\overline{X} - 1}{\frac{1}{\sqrt{4}}} < \frac{0 - 1}{\frac{1}{\sqrt{4}}}  \right) = P(Z < -2) \approx 0.023$
...
![Error Tipo I y Error Tipo II](https://whatsthebigdata.files.wordpress.com/2017/01/errors.jpg)
...
$$= P\left( \textbf{X} \in RC_{\delta} | \theta \in \Omega_0 \right) = Potencia\left(\theta | \theta \in \Omega_0 \right)$$
...
$F_{X_{(n)}}(2.9) = \left(\frac{2.9}{\theta}\right)^{n}  \quad \text{y} \quad F_{X_{(n)}}(3.9) = \left(\frac{3.9}{\theta}\right)^{n} = 1$
...
$\text{Potencia}(\theta) = \begin{cases} 1 \quad si \quad 0 < \theta \leq 2.9 \\ \left(\frac{2.9}{\theta}\right)^{n} \quad si \quad 2.9 < \theta \leq 3.9 \\ \left(\frac{2.9}{\theta}\right)^{n} + 1 - \left(\frac{3.9}{\theta}\right)^{n} \quad si \quad \theta > 3.9 \end{cases}$
...
$\beta(5) = 1 - Potencia(5) = 1 - \left( \left(\frac{2.9}{5}\right)^{10} + 1 - \left(\frac{3.9}{5}\right)^{10} \right) = 0.079$
...
Contrastes de hipótesis - todo el resto
