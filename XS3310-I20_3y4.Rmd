...
**Definición**. Se define la **función Score** como:
...
$$f(x|\mu) = \dfrac{1}{\sqrt{2\pi\sigma^2}} \ e^{-\frac{1}{2}\left(\frac{x-\mu}{\sigma}\right)^2}$$

Vea que
\begin{align*}
&\dfrac d{d\mu}\int_{\mathbb R} f(x|\mu)dx  = \int_{\mathbb R}f'(x|\mu)dx = \int_{\mathbb R} \dfrac 1{\sqrt{2\pi\sigma^2}}\ e^{-\frac{1}{2}\left(\frac{x-\mu}{\sigma}\right)^2} \cdot \dfrac {-2(x-\mu)}{2\sigma^2} dx\\
& = -\dfrac{1}{\sigma^3} \underbrace{\int_{\mathbb R}\dfrac{u}{\sqrt{2\pi}}e^{-\frac{u}2}du}_{\mathbb E[N(0,1)]} = 0 \quad \text{usando el cambio de variable } \dfrac{x-\mu}\sigma
\end{align*}
...
**IMPORTANTE**. Suponga que $X = (X_1,\dots,X_n)$ muestra de $f(x|\theta)$ donde $f$ satisface las condiciones anteriores. La información de Fisher de la muestra es 
...
En general, para buscar la eficiencia al estimar un parámetro se requiere determinar el estimador insesgado $\hat{\theta}$ que tiene varianza mínima. 

**Teorema.** (**Desigualdad de Cramer-Rao**). Sea $X_{1}, X_{2}, ... ,X_{n}$ una muestra aleatoria sobre una población con parámetro desconocido $\theta$ con función de densidad $f_{X}(x|\theta)$, cuyo dominio no depende de $\theta$. Sea $T$ cualquier estimador de $\theta$, con $m(\theta)=\mathbb{E}_{\theta}(T)$ y asuma que $m$ es diferenciable. Entonces siempre se cumple la siguiente desigualdad:

 $$\text{Var}_\theta(T)\geq \dfrac{[m'(\theta)]^2}{I_n(\theta)} =\dfrac{[m'(\theta)]^2}{nI(\theta)} .$$
 
 Si $T$ es insesgado, entonces la desigualdad se simplifica a 

 $$\text{Var}_\theta(T)\geq \dfrac{1}{I_n(\theta)} =\dfrac{1}{nI(\theta)} .$$

**NOTA:** Si existe un estimador $T$ tal que $Var(T) = \dfrac{1}{nI(\theta)}$,  entonces le decimos que es un estimador insesgado de varianza mínima para $\theta$. 

---
  
# Eficiencia
	
**Ejemplo:** Sea $X_{1}, X_{2}, ... , X_{n}$ una muestra aleatoria de una población Normal con media $\mu$ y varianza $\sigma^{2}$. Demuestre que $\overline{X}$ es un estimador insesgado de varianza mínima (EIVM) para $\mu$.
	
**Solución:** Sabemos que $\mathbb{E}[\overline{X} ] = \mu$. Basta probar que tiene varianza mínima. 
Sabemos que en este caso $f_{X}(x) = \frac{1}{\sqrt{2\pi}\sigma}e^{-\frac{(x-\mu)^2}{2\sigma^2}}$.

* $\lambda(x|\mu) = -\dfrac 12 \ln (2\pi\sigma^2)-\dfrac 1{2\sigma^2}(x-\mu)^2$.

* $\lambda'(x|\mu) = -\dfrac 1{2\sigma^2}2(x-\mu)\cdot -1 = \dfrac{x-\mu}{\sigma^2}$.

* $\lambda''(x|\mu) = -\dfrac 1{\sigma^2}$.

---

* $I(\mu) = - \mathbb{E}(\lambda''(x|\mu)) = \dfrac 1{\sigma^2}$
* $I_{n}(\mu) = nI(\mu) = \dfrac n{\sigma^2}$
* Cota de Cramer-Rao = $\dfrac{\sigma^2}n$


De antemano ya sabiamos que $Var(\overline{X}) = \dfrac{\sigma^2}{n}$ que como vemos es igual a la cota de Cramer-Rao para $\mu$, por lo que podemos concluir que $\overline{X}$ es un estimador insesgado de varianza mínima para $\mu$.
...
c) Encuentre la varianza del estimador de la parte anterior. 
...
Sean $X_1, . . . , X_n, X_{n + 1}, . . . , X_{n+m}$ variables aleatorias independientes e idénticamente distribuidas con densidad Normal de media $\mu$ y varianza $\sigma^2_X$. Se desea estimar $\mu$, pero los valores individuales de la variables se han extraviado y se dispone sólo de las medias:

$\bar{X_1}=\frac{1}{n}\sum_{i=1}^{n}X_i$

$\bar{X_2}=\frac{1}{m}\sum_{i=n+1}^{n+m}X_i$
