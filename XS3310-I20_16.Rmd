...
Pruebas de hipótesis para el caso de diferencia de medias o cociente de varianzas. Bootstrap para contrastes.
...
\mathbb P[U\geq c|\mu_1=\mu_2,\sigma^2] = 1-\mathbb P[U\leq c|\mu_1=\mu_2,\sigma^2] \leq \alpha_0 \\ 
& \implies  P[U\leq c|\mu_1=\mu_2,\sigma^2] \geq 1-\alpha_0 \\
& \implies c = t_{1-\alpha_0,n+m-2} 
\end{align*}
...
\[v\geq F_{1-\frac{\alpha_0}2, m-1,n-1} \implies P_{m-1,n-1}(V \leq v \vert \sigma_1 = \sigma_2 ) \geq 1-\dfrac{\alpha_0}2 \implies \alpha_0\geq 2 (1- P_{m-1,n-1}(V \leq v \vert \sigma_1 = \sigma_2 )) \]
...
#
d <- data.frame(c(trat, cont), c(rep("t", length(trat)), rep("c", length(cont))))
names(d) <- c("dias", "grupo")

knitr::kable(d)

# BoxPlot de los días de superviviencia por grupo
boxplot(d$dias ~ d$grupo,
  las = 1, ylab = "Días",
...
_Interpretación:_ No se puede rechazar la hipótesis nula si tomamos un error tipo I menor a `r p`. 
