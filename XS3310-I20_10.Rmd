...
$$\sqrt{n}\left(g\left(\overline{X} - g(\mu)\right)\right) \stackrel{d}{\to} N\left(0,\left( g^{\prime}(\mu) \sigma \right)^2\right)$$
...
$$\begin{equation*} \begin{aligned} g^{\prime}(u)^{2} \sigma^{2}(\mu) &=1 \\ \Longrightarrow g^{\prime}(\mu) &=\frac{1}{\sigma(\mu)} \quad(\sigma(\mu)>0) \\ \Longrightarrow g(\mu) &=\int_{a}^{\mu} \frac{d x}{\sigma(x)} d x\end{aligned}\end{equation*}$$
...
$$\left[\sin\left(\frac{1.179}{2}\right)^2,\sin\left(\frac{1.963}{2}\right)^2\right] \\= [0.309, 0.691]$$
