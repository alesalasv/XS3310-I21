...
* Horas consulta de Andrea: cambian a Jueves de 1 a 3pm

* Repo actualizado: https://malfaro2.github.io/XS3310-I20/

* Notas del examen se postearán en Mediación Virtual la otra semana

* Hoy iniciamos el Tema II y luego vendrá el tema III para completar el material del II Parcial.-->
...
En el primer caso se tienen dos valores $Li$ y $Ls$ que constituyen los límites inferior y superior del intervalo. Es decir, el intervalo $\left[Li, Ls\right]$ contiene el valor de $\theta$ con una confianza del $(1-\alpha)\cdot 100\%$. 
	
En el segundo caso, se determina solamente un límite que puede ser inferior $(Li)$ o superior $(Ls)$, dependiendo del interés del estudio. Se tiene así intervalos de la forma $\left[ Li, +\infty \right[$ o $\left] -\infty, Ls \right]$. Los valores $\pm \infty$ pueden ser sustituidos por el mínimo o máximo valor del dominio del parámetro de interés. 
...
* $\overline{X}$ resume la información de la muestra aleatoria, pues está en términos de un estadístico suficiente y además sabemos que se distribuye $N(\mu, \frac{\sigma^2}{n})$. Ahora debemos encontrar alguna transformación de $\overline{X}$ que incluya a $\mu$ y cuya distribución no dependa de $\mu$. 
	
---

### Técnica del pivote

* $U = \frac{\overline{X} - \mu}{\frac{\sigma}{\sqrt{n}}}$, se distribuye $N(0,1)$. U cumple con las propiedades de un pivote pues está definida en términos de la muestra y del parámetro desconocido $\mu$ al igual que su distribución ya no depende de $\mu$. 
...
**Solución:** En este caso podemos utilizar como pivote $U = \frac{Y}{\beta}$ ya que está en términos de la variable aleatoria y de $\beta$ al igual que se puede demostrar que su distribución no depende de $\beta$. En este caso $U \sim Exp(1)$. 
...
$P(a \leq U \leq b) = P\left( -\ln(0.95) \leq \frac{Y}{\beta} \leq -\ln(0.05) \right)=$ 
	
$P\left( \frac{-1}{\ln(0.95)} \geq \frac{\beta}{Y} \geq \frac{-1}{\ln(0.05)} \right) = P\left( \frac{-Y}{\ln(0.95)} \geq \beta \geq \frac{-Y}{\ln(0.05)} \right) = 0.90$
	
Por lo tanto tenemos que $Li = \frac{-Y}{\ln(0.05)}$ y $Ls = \frac{-Y}{\ln(0.95)}$ por lo que podemos concluir que con una confianza del $90\%$ el intervalo $\left[ \frac{-Y}{\ln(0.05)}, \frac{-Y}{\ln(0.95)} \right]$ incluye el valor de $\beta$. 
...
**Solución:** Primero debemos encontrar una cantidad pivote satisfactoria. En este caso podemos utilizar $U = \frac{X}{\theta}$ como pivote ya que cumple ambas condiciones. Se puede demostrar que $U \sim Unif(0,1)$ por medio de la segunda técnica de transformaciones: 
	
Sea $U = h(X) = \frac{X}{\theta}$, por lo tanto $h^{-1}(U) = U\theta \quad \Rightarrow \frac{dh^{-1}(u)}{du} = \theta$

Aplicando la técnica tenemos:
	
$f_{U}(u) = f_{X}(u\theta) \cdot \theta = \frac{1}{\theta} \cdot \theta = 1$
...
$P(U \leq b) = P\left( \frac{X}{\theta} \leq 0.95  \right) = P\left( \theta \geq \frac{X}{0.95}  \right) = 0.95$
...
Fórmulas para las estimaciones por intervalo más comunes (media, diferencias de medias, variancias, etc., para distribuciones normales), bootstrap y contrastes de hipótesis.
