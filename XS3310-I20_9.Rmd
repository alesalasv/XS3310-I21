...
Igual que siempre, nuesto primer paso es obtener una cantidad pivote. Ya conocemos como se distribuyen los promedios muestrales, por lo que podemos partir de ahí. Recordemos que $\overline{X} \sim N(\mu_{1}, \dfrac{\sigma^{2}_{1}}{n})$ y $\overline{Y} \sim N(\mu_{2}, \dfrac{\sigma^{2}_{2}}{m})$. Como ambos son Normales entonces una combinación lineal de ellos también sería Normal; necesitamos una combinación lineal de estos dos que tenga como parámetro a $\mu_{1} - \mu_{2}$. 
...
$(\overline{X} - \overline{Y}) \pm t_{\frac{\alpha}{2}, n+m-2} \cdot S_{p} \sqrt{\frac{1}{n} + \dfrac{1}{m}}$ 
	
Concluimos que con una confianza del $(1-\alpha)\%$ el intervalo $(\overline{X} - \overline{Y}) \pm t_{\frac{\alpha}{2}, n+m-2} \cdot S_{p} \sqrt{\dfrac{1}{n} + \dfrac{1}{m}}$ contiene el verdadero valor de $\mu_{1} - \mu_{2}$. 
...
$\dfrac{1}{\sigma}f(\dfrac{x-\mu}{\sigma})$|Posición-Escala| $\dfrac{\bar{X}-\mu}{S}$|
...
$$f(x) = \dfrac{1}{\sqrt{2\pi}} \exp{(\dfrac{-1}{2}(x-\mu)^2)} = g(z)$$
en donde $z = x-\mu$, y $g(z)=\dfrac{1}{\sqrt{2\pi}} \exp{(\dfrac{-1}{2}(z)^2)}$.
...
$$f(x) = \dfrac{1}{\sigma}\dfrac{1}{\sqrt{2\pi}} \exp{(\dfrac{-1}{2}(x/\sigma)^2)} = \dfrac{1}{\sigma} g(z)$$
en donde $z = x/\sigma$, y $g(z)=\dfrac{1}{\sqrt{2\pi}} \exp{(\dfrac{-1}{2}(z)^2)}$.

---

# Pivotes para distribuciones con forma posición-escala

Finalmente, para $\dfrac{1}{\sigma}f(\dfrac{x-\mu}{\sigma})$:

Sea $X \sim N(\mu, \sigma^2)$, entonces la función de densidad es:

$$f(x) = \dfrac{1}{\sigma}\dfrac{1}{\sqrt{2\pi}} \exp{(\dfrac{-1}{2}(\dfrac{x-\mu}{\sigma})^2)} = \dfrac{1}{\sigma} g(z)$$
en donde $z = \dfrac{x-\mu}{\sigma}$, y $g(z)=\dfrac{1}{\sqrt{2\pi}} \exp{(\dfrac{-1}{2}(z)^2)}$.
