...
* Asignación de la tarea o examen corto 1
...
> **Definición 5.4.** Momento muestral. Si $X_{1}, X_{2}, ... , X_{n}$ es una muestra aleatoria de una distribución conocida, entonces se define el \textit{k}-ésimo momento muestral como:
...
$\begin{matrix} \mu_{1}^{\prime} = E(X) = \alpha\beta & m_{1}^{\prime} = \overline{X} \\ \mu_{2}^{\prime} = E(X^{2}) = \alpha \beta^{2} + (\alpha\beta)^{2} & m_{2}^{\prime} = \frac{1}{n} \sum_{j=1}^{n} X_{j}^{2} = \overline{X^2} \end{matrix}$
...
Supongamos que obtuvimos una muestra aleatoria de tamaño tres donde observamos los siguientes datos: 4,4,3,4. ¿Cuál es el valor de $\theta$ que máximiza la función de verosimilitud? 
...
$\mathcal{L}(\theta) = P(Y=4|\theta) \cdot P(Y=4|\theta) \cdot P(Y=3|\theta) \cdot P(Y=4|\theta) = P(Y=4|\theta)^{3} \cdot P(Y=3|\theta)$
...
<!--# Asignación examen corto 1

* Dinámica similar a los quices de contínuos, excepto que no pediremos que vengan en Markdown, aunque pueden hacerlo si quieren. 

* El formato obligatorio de entrega en un pdf, pero siempre pueden tomarle una foto a lo que escriben y convertirlo. Por favor practiquen antes del día de la evaluación para asegurarse que saben cómo convertir una foto de celular a un pdf y subirlo a Mediación Virtual.

* Fecha de entrega: sábado 25 de abril a la media noche. Tienen 24 horas para resolverlo.

* Instrucciones están posteadas en Mediación Virutal desde ya, y los problemas se postearán hoy a la media noche.

* Seguir las instrucciones **al pie de la letra** vale un 50% de la evaluación.
